...
void main() {
  ...
  group('parseCueSheet — album metadata', () {
    ...
    test('custom REM field',
        () => expect(sheet.remComments['COMMENT'], 'Ripped with EAC'));
  });
  ...
  group('parseCueSheet — track parsing', () {
    ...
    test('track 01 number',
        () => expect(sheet.files[0].tracks[0].trackNumber, 1));
    test('track 01 title',
        () => expect(sheet.files[0].tracks[0].title, 'First Song'));
    test('track 01 performer',
        () => expect(sheet.files[0].tracks[0].performer, 'Solo Artist'));
    test('track 01 songwriter',
        () => expect(sheet.files[0].tracks[0].songwriter, 'Solo Writer'));
    test('track 01 isrc',
        () => expect(sheet.files[0].tracks[0].isrc, 'ZZABC1234567'));
    test('track 01 flags', () {
  ...
  group('parseCueSheet — INDEX handling', () {
    ...
    test(
        'INDEX 00 parsed', () => expect(track1.indices.containsKey(0), isTrue));
    test(
        'INDEX 01 parsed', () => expect(track1.indices.containsKey(1), isTrue));
    test(
        'INDEX 01 value', () => expect(track1.startTime, parseMsf('00:02:00')));
  });
  ...
  group('parseCueSheet — track type detection', () {
    ...
    test('MODE1/2352', () {
      final cue =
          'FILE "a.bin" BINARY\n  TRACK 01 MODE1/2352\n    INDEX 01 00:00:00\n';
      expect(parseCueSheet(cue)!.files[0].tracks[0].trackType,
    ...
    test('CDG', () {
      final cue =
          'FILE "a.bin" BINARY\n  TRACK 01 CDG\n    INDEX 01 00:00:00\n';
      expect(
          parseCueSheet(cue)!.files[0].tracks[0].trackType, CueTrackType.cdg);
    });
  });
  ...
  group('parseCueSheet — multiple FILE entries', () {
    ...
    test(
        'file 2 filename', () => expect(sheet.files[1].filename, 'disc2.aiff'));
    test('file 2 track title',
        () => expect(sheet.files[1].tracks[0].title, 'Track 3'));
  });
  ...
  group('parseCueSheet — spec coverage additions', () {
    ...
    test('DATA track type', () {
      final cue =
          'FILE "a.bin" BINARY\n  TRACK 01 DATA\n    INDEX 01 00:00:00\n';
      expect(
          parseCueSheet(cue)!.files[0].tracks[0].trackType, CueTrackType.data);
    });
    ...
    test('FLAGS DATA', () {
      final cue =
          'FILE "a.wav" WAVE\n  TRACK 01 AUDIO\n    FLAGS DATA\n    INDEX 01 00:00:00\n';
      expect(
          parseCueSheet(cue)!.files[0].tracks[0].flags, contains(CueFlag.data));
    });
